...
class Grafo {
  ...
  get esConexo() {
    ...
    // grafo no es conexo.
    return !includes(visitar(first(this.nodos)), undefined);
  }
  ...
  eliminarArista(origen, destino, direccion = Direccion.salida) {
    ...
    else if (direccion === Direccion.entrada && this.esDirigido) {
      return this.eliminarArista(destino, origen, Direccion.salida);
    }
  ...
  caminoMasCorto(origen, destino) {
    const n = this.cantidad;
    let noVisitados = new Set(this.nodos);
  ...
  euleriano(tipo = Trayecto.camino) {
    ...
    const hierholzer = (origen) => {
      // Stack de nodos que forman un subtrayecto euleriano.
      let subtrayecto = [origen];
