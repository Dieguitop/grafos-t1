...
class Grafo {
  ...
  hamiltoniano(tipo) {
    const permutador = {
      ...
      generar: function* (lista) {
        if (lista.length === 1) {
          ...
          for (let permutacion of permutador.generar(resto)) {
            for (const j of lista.keys()) {
              // Obtiene la porción desde 0 hasta j de la permutación.
              let inicio = permutacion.slice(0, j);

              // Obtiene la porción desde j hasta el final de la permutación.
      ...
      /**
       * Comprueba si la permutación es un trayecto hamiltoniano válido en el
       * grafo.
       *
       * @param {number[]} permutacion - Permutación de la lista de nodos.
       * @returns {boolean} `true` si la permutación es un trayecto hamiltoniano
       * válido en el grafo, `false` en caso contrario.
       */
      esHamiltoniano: (permutacion) => {
        for (let i = 0; i < permutacion.length - 1; i++) {
          // Comprueba que exista una arista entre cada par de nodos (i, i + 1).
          if (!this.existeArista(permutacion[i], permutacion[i + 1], Direccion.salida)) {
            return false;
          }
        }
    ...
    for (let permutacion of permutador.generar(this.nodos)) {
      // Si el tipo de trayecto es camino, o no se especificó ninguno, comprobar
      // que la permutación sea hamiltoniana en el grafo. En caso verdadero,
      // retornar la permutación.
      if ((tipo === Trayecto.camino || tipo == null) && permutador.esHamiltoniano(permutacion)) {
        return permutacion;
      }
      ...
      // Si el tipo de trayecto que se busca es un ciclo, o no se especificó
        ...
        // Si la permutación (modificada) es hamiltoniana, retornarla.
        if (permutador.esHamiltoniano(permutacion)) {
          return permutacion;
        }
      }
    }
