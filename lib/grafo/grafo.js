...
class Grafo {
  ...
  caminoMasCorto(origen, destino) {
    ...
    while (!isEmpty(noVisitados)) {
      ...
      const actual = [...noVisitados].reduce(
        (menor, nodo) => (menor = distancia[nodo] < distancia[menor] ? nodo : menor)
      );
      ...
      for (const { nodo: siguiente, peso = 1 } of this.adyacentes(actual, Direccion.salida)) {
    ...
    if (!Number.isFinite(distancia[destino])) {
