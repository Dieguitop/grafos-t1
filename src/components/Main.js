...
const Main = () => {
  ...
  const grafoClass = (from, to, peso) => {
    ...
    const listaDeAristas = [new Arista(numberFrom, numberTo, numberPeso)];
  ...
  const saveData = () => {
    ...
    if (validation === true) {
      ...
      const { camino: shortPathGraph, distancia: distanceShortes } = gra.caminoMasCorto(
        distanceFrom,
        distanceTo
      );
  ...
  const handleOnAdd = () => {
    ...
    rows[len].text.length > 0
      ? setRows(rows.concat(defaultState))
      : swal(
          "Error en los nodos",
          "Campos vacíos, por favor asigne nombre a los nodos creados",
          "error"
        );
  };
  ...
  const handleOnAddLink = () => {
    ...
    links[len].from.length > 0 || links[len].to.length > 0 || links[len].text.length > 0
      ...
      : swal(
          "Error en los links",
          "Campos vacíos en los links, por favor complete los datos antes de continuar",
          "error"
        );
  };
  ...
  return (
    <Box display="flex" className="main_view">
      <div className="main_options">
        <Button m={10} variant="contained" color="secondary" onClick={handleOnAdd}>
          Agregar
        </Button>
        {rows.map((row, index) => (
          <Row
        ...
        <hr />
        <Button m={10} variant="contained" color="grey" onClick={handleOnAddLink}>
          Agregar link
        </Button>
        ...
        <div>
          {links.length < 1 || rows.length < 1 ? (
            <Button m={10} className="main_row" variant="contained" color="primary" disabled>
              GUARDAR DATOS
              <SaveAlt />
            </Button>
          ) : (
        ...
        <div className={doneFetch === true ? "main_isMatriz" : "main_noMatriz"}>
          <Button variant="contained" color="gray" type="submit" className="main_buttonMargin">
            <RefreshIcon />
          </Button>
          <div className="item">
            {matrizDeCamino &&
        ...
        <div className={doneFetchHamEul === true ? "main_isMatriz" : "main_noMatriz"}>
          <div className="distanceFromTo">
            <p>
        ...
        <div className={doneFetchFlujoMaximo === true ? "main_isMatriz" : "main_noMatriz"}>
        ...
        <div className={doneFetchArbol === true ? "main_isMatriz" : "main_noMatriz"}>
          <Button variant="contained" color="gray" type="submit">
            <RefreshIcon />
          </Button>
          <div className="main_contentInside">
            {arbolGenerador && <Content className="hola" data={rows} linksData={arbolGenerador} />}
          </div>
        </div>
        ...
        <Button type="button" onClick={refreshPage}>
          ...
          Recargar la página{" "}
          <span>
            <CachedIcon className="main_reloadIcon" />
          </span>{" "}
        </Button>
      </div>
